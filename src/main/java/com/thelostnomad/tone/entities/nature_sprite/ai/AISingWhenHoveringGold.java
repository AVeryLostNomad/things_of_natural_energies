...
public class AISingWhenHoveringGold extends NatureSpriteAI {

    private final NatureSpriteEntity parentEntity;

    public AISingWhenHoveringGold(NatureSpriteEntity ghast) {
    ...
    public boolean shouldExecute() {
        if (parentEntity.getStamina() >= (this.parentEntity.getSpeciesHelper().getStamina() * 0.75) && parentEntity.flags.containsKey("gold")) {
            return true;
        }
        return false;
    }
    ...
    public void updateTask() {
        if (parentEntity.world.rand.nextInt(100) < 5) {
            ...
            for (EntityPlayerMP ep : entities) {
                TonePacketHandler.sendTo(new SpriteSingPacket(this.parentEntity.getPosition(), SoundEvents.BLOCK_NOTE_SNARE), ep);
            }
